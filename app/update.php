<?php
	...
	if(empty($_SESSION['username']))
		...
		if(isset($_POST['register_individuals'])) {
			...
			//$sale = $_POST['sale'];
			try {
				$stmt = $connect->prepare('UPDATE room_rental_registrations SET fullname = ?,  email = ?, mobile = ?, alternat_mobile = ?, plot_number = ?,  state = ?, city = ?, area = ?, address = ?,  deposit = ?, description = ?, accommodation = ?, other = ?, user_id = ?  WHERE id = ?');
				$stmt->execute(array(
					$fullname,
					$email,
					$mobile,
