  ...
  //end of location based search
    ...
    try {
      //foreach ($keyword as $key => $value) {
        ...
        $stmt = $connect->prepare("SELECT * FROM room_rental_registrations_apartment WHERE area IN $concats OR area IN $loc OR state IN $concats OR state IN $loc OR city IN $concats OR city IN $loc OR address IN $concats OR address IN $loc OR rooms IN $concats OR landmark IN $concats OR landmark");
        $stmt->execute();
        $data2 = $stmt->fetchAll(PDO::FETCH_ASSOC);

        $stmt2 = $connect->prepare("SELECT * FROM room_rental_registrations WHERE area IN $concats OR area IN $loc OR state IN $concats OR state IN $loc OR city IN $concats OR city IN $loc OR rooms IN $concats OR address IN $concats OR address IN $loc OR landmark ");
        $stmt2->execute();
        $data8 = $stmt2->fetchAll(PDO::FETCH_ASSOC);
      ...
      <div class="container">
        <div class="row">
          <div class="col-lg-12 text-center">
            ...
            <h3 class="section-subheading text-muted"> <font color="black">Search Rooms or Appartment for purchase.</font></h3>
          </div>
        </div>
        <div class="row">
          <div class="col-md-12">
            ...
            <?php
              ...
              if(count($data) !== 0){
                echo "<h2 class='text-center'>List of Properties</h2>";
              }else{
                 ...
                 echo '<div class="card card-inverse card-info mb-3" style="padding:1%;">         
                  <div class="card-block">';
                    ...
                    echo '</div>
                      ...
                      <h4 class=>Property Details</h4>';
                        ...
                        echo '<p><b>Number of  Rooms: </b>'.$value['rooms'].'</p>';
                          ...
                          echo '<p> <b> Property image: </b> </p> <p> <img src="'.$value['image'].'" width="100"></p>';
                        }
                    ...
                    echo '</div>
                      ...
                      echo '<a class="btn btn-warning float-right" href="../app/appointment.php?id='.$value['id'].'&act=';if(!empty($value['own'])){ echo "ap"; }else{ echo "indi"; } echo '">Book Appointment</a><br><br>';
                }
              ?>              
          </div>
        </div>
